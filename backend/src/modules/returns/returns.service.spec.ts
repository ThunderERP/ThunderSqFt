...
const mockPrisma = {
  ...
  return:   { create: jest.fn(), findUnique: jest.fn(), update: jest.fn(), findMany: jest.fn(), findFirst: jest.fn(), count: jest.fn() },
...
const makeOrder = (status: OrderStatus) => ({
  id: 1, status, customerId: 1, isActive: true, createdBy: 1,
  invoice: { id: 1, totalAmount: '1000', paidAmount: '1000', status: 'PAID' },
  items: [{ id: 1, productId: 1, quantity: 5, orderId: 1, unitPrice: '200', gstPct: '18', discountPct: '0', lineTotal: '1000' }],
});

const makeReturn = (type: ReturnType) => ({
  id: 1, orderId: 1, invoiceId: 1, productId: 1, quantity: 2,
  type, status: ReturnStatus.REQUESTED, reason: 'Damaged',
  createdBy: 1, createdAt: new Date(), updatedAt: new Date(),
  refundAmt: null, resolvedAt: null, resolvedBy: null,
...
describe('ReturnsService', () => {
  ...
  describe('create', () => {
    it('throws NotFoundException for missing order', async () => {
      ...
      await expect(
        service.create({ orderId: 99, productId: 1, quantity: 1, type: ReturnType.REFUND, reason: 'broken' }, 1),
    ...
    it('throws UnprocessableEntityException for non-DELIVERED order', async () => {
      ...
      await expect(
        service.create({ orderId: 1, productId: 1, quantity: 1, type: ReturnType.REFUND, reason: 'broken' }, 1),
    ...
    it('throws BadRequestException if product was not in order', async () => {
      ...
      await expect(
        service.create({ orderId: 1, productId: 99, quantity: 1, type: ReturnType.REFUND, reason: 'broken' }, 1),
    ...
    it('throws BadRequestException if return quantity exceeds ordered quantity', async () => {
      ...
      await expect(
        service.create({ orderId: 1, productId: 1, quantity: 99, type: ReturnType.REFUND, reason: 'broken' }, 1),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('process', () => {
    it('throws NotFoundException when return does not exist', async () => {
      ...
      await expect(
        service.process(99, { status: ReturnStatus.APPROVED }, 1),
      ).rejects.toThrow(NotFoundException);
    });

    it('throws UnprocessableEntityException if return is already processed', async () => {
      mockPrisma.$transaction.mockImplementation(async (fn: Function) => fn(mockPrisma));
      mockPrisma.return.findUnique.mockResolvedValue({ ...makeReturn(ReturnType.REFUND), status: ReturnStatus.COMPLETED });
      await expect(
        service.process(1, { status: ReturnStatus.APPROVED }, 1),
      ).rejects.toThrow(UnprocessableEntityException);
    });
    ...
    it('calls atomicAdd (stock restored) for RETURN type when approved', async () => {
      ...
      mockPrisma.return.update.mockResolvedValue({ ...makeReturn(ReturnType.RETURN), status: ReturnStatus.COMPLETED });
    ...
    it('calls atomicDeduct (stock sent) for REPLACEMENT type when approved', async () => {
      ...
      mockPrisma.order.update.mockResolvedValue({});
      mockPrisma.return.update.mockResolvedValue({ ...makeReturn(ReturnType.REPLACEMENT), status: ReturnStatus.COMPLETED });
      mockPrisma.auditLog.create.mockResolvedValue({});
    ...
    it('does not touch inventory for REJECTED return', async () => {
      mockPrisma.$transaction.mockImplementation(async (fn: Function) => fn(mockPrisma));
      mockPrisma.return.findUnique.mockResolvedValue(makeReturn(ReturnType.RETURN));
      mockPrisma.return.update.mockResolvedValue({ ...makeReturn(ReturnType.RETURN), status: ReturnStatus.REJECTED });
      mockPrisma.auditLog.create.mockResolvedValue({});
